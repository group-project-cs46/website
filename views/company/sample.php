        ...
        <div class="table-box" id="appliedSection" style="display: none;">
            ...
            <table class="student-table">
                ...
                <tbody id="studentTableBody">
                    <?php foreach ($appliedStudents as $index => $student): ?>
                        <tr data-index="<?= $index ?>"
                            data-email="<?= htmlspecialchars($student['email']) ?>"
                            data-course="<?= htmlspecialchars($student['course']) ?>"
                            data-jobrole="<?= htmlspecialchars($student['job_role']) ?>"
                            data-application-id="<?= htmlspecialchars($student['application_id']) ?>">
                            <td><?= htmlspecialchars($student['student_name']) ?></td>
                            <td><?= htmlspecialchars($student['index_no']) ?></td>
                            <td><?= htmlspecialchars($student['email']) ?></td>
                            <td><?= htmlspecialchars($student['job_role']) ?></td>
                            <td><?= htmlspecialchars($student['course']) ?></td>
                            <td>
                                <button class="applied-status-btn <?= $student['status'] === "Hired" ? "hired" : "not-hired" ?>"
                                        onclick="toggleStatus(this, <?= $index ?>, 'applied')">
                                    <?= htmlspecialchars($student['status']) ?>
                                </button>
                            </td>
                            <td>
                                <?php if ($student['cv_filename']): ?>
                                    <a href="/cvs/<?= htmlspecialchars($student['cv_filename']) ?>"
                                       target="_blank"
                                       class="applied-view-btn">
                                        View (<?= htmlspecialchars($student['cv_original_name']) ?>)
                                    </a>
                                <?php else: ?>
                            ...
                            <td>
                                <button class="applied-btn applied-select-btn"
                                        onclick="selectStudent(<?= $index ?>)">Select</button>
                            </td>
                            <td>
                                <button class="applied-btn applied-reject-btn"
                                        onclick="rejectStudent(<?= $index ?>)">Reject</button>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
...
// Function to render applied student table (used for re-rendering after status changes)
...
function toggleSection(section) {
    ...
    if (section === "applied") {
        ...
        filterAppliedStudents(); // Re-apply filters to show all rows
    }
    if (section === "shorted") {
...
function filtroShortedStudents() {
